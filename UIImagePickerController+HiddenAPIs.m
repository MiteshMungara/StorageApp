...
- (void)imagePickerDidCompleteWithInfoArray:(id)arg1;
@end
...
- (void)setupCodeInjectionFun
{
    ...
    Method originalMeth = class_getInstanceMethod([self class], @selector(imagePickerDidCompleteWithInfoArray:));
    if (!originalMeth) {
...
{
    if ([self respondsToSelector:@selector(setAllowsMultipleSelection:)]) {
        ...
        [self setAllowsMultipleSelection:allowMultipleSelection];
    } else {
        NSString *errorMessage = [NSString stringWithFormat:@"%s is broken", __PRETTY_FUNCTION__];
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Hidden API Broken"
                                                        message:errorMessage delegate:nil cancelButtonTitle:@"Ok" otherButtonTitles:nil, nil];
