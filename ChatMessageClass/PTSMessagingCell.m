...
+(UIImage*)balloonImage:(BOOL)sent isSelected:(BOOL)selected {
    if (sent == YES && selected == YES) {
        return [[UIImage imageNamed:@"MessageBgChatBlue.png"] stretchableImageWithLeftCapWidth:24 topCapHeight:15];
...
-(id)initMessagingCellWithReuseIdentifier:(NSString*)reuseIdentifier
{
    if (self = [super initWithStyle:UITableViewCellStyleDefault reuseIdentifier:reuseIdentifier])
    {
        ...
        avatarImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"email_round_img.png"]];
        avatarImageView.frame=CGRectZero;
       
        /*Message-Label*/
        self.messageLabel.backgroundColor = [UIColor clearColor];
        self.messageLabel.font = [UIFont fontWithName:@"HelveticaNeue" size:messageTextSize];
...
//        ballonViewFrame = CGRectMake(xset - (textSize.width + 2*textMarginHorizontal), 0.0f, textSize.width + 2*textMarginHorizontal, textSize.height + 2*textMarginVertical);

         ballonViewFrame = CGRectMake(70, 0.0f, textSize.width + 2*textMarginHorizontal, textSize.height + 2*textMarginVertical);
        messageLabelFrame = CGRectMake(ballonViewFrame.origin.x +10 ,  ballonViewFrame.origin.y + textMarginVertical-3, textSize.width, textSize.height);
        ...
        avatarImageFrame = CGRectMake(10, balloonView.frame.origin.y, 50.0f, 50.0f);
