...
//    NSLog(@"hosting:%@ centre:%@ radius:%f",NSStringFromCGRect(layerHostingView.frame),NSStringFromCGPoint(layerHostingView.pieCenter),(layerHostingView.pieRadius));
    ...
    //**************       labels
    int y = 5;
    NSArray *labelsName=[[NSArray alloc]initWithObjects:@"audios",@"videos",@"images",@"files", nil];
    for (int i=0;i<labelsName.count;i++)
    {
        ...
        UILabel *lbl = [[UILabel alloc] initWithFrame:CGRectMake(25, y, 40, 25)];
        lbl.text = [NSString stringWithFormat:@"%@",[labelsName objectAtIndex:i]];
